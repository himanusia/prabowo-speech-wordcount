...
<title>prabowo.speeches / corpus index</title>
<style>
...
.tag{color:var(--amber)}
.mbgrow{display:grid;grid-template-columns:78px 1fr 40px;gap:10px;padding:6px 0;border-bottom:1px solid var(--line);font-size:9px;color:var(--muted)}
.mbgrow b{color:var(--ink);font-weight:500}
...
<body>
<div class="shell">
<header class="top"><div><span class="dim">~/research/</span><span class="green">prabowo-corpus</span><span class="dim"> $ ./index</span></div><div class="ok green" id="stamp">dataset loaded</div></header>
<main>
<section class="overview"><div><div class="eyebrow">// YOUTUBE CAPTIONS · ID · DEDUPLICATED EVENTS</div><h1>prabowo.speeches<br>/ corpus index</h1><p class="lede">Setiap pidato dihitung sebagai satu event, walaupun diunggah banyak kanal. Angka kata, topik, MBG, dan framing dihitung dari event kanonik.</p></div><div class="metrics">
...
<div class="head" style="margin-top:22px"><h2>mbg</h2><span id="mbgMeta">—</span></div>
<p class="sub">exact MBG + frasa penuh + konteks makan bergizi / SPPG.</p>
<div id="mbg"></div>
...
<script>
const DATA=__DATA__;
const STOP=new Set("yang dan di ke dari untuk dengan ini itu pada dalam adalah akan ada atau bagi sebagai oleh para sudah sangat juga lebih tidak ya kan lagi hanya semua bisa bahwa telah dapat atas terhadap menjadi secara karena namun maka kalau jika ketika seperti tentang antara kepada daripada sampai serta pun saya kita kami mereka kalian anda saudara sekalian terima kasih tapi jadi baik banyak besar hari sekarang tadi sama punya mau boleh mungkin masih nanti ingin selalu iya terus mana kali waktu orang dia pak apa satu seluruh hadir harus tahun negara bangsa".split(' '));
...
function mbg(){
  const withMbg=events.filter(e=>e.mbg.mbg_policy_signal_count>0||e.mbg.mbg_ambiguous_count>0);
  $('mbgMeta').textContent=withMbg.length+'/'+DATA.sources.length+' event';
  $('mbg').innerHTML=withMbg.map(e=>{
    const flags=e.mbg.mbg_ambiguous_count?' <span class="tag">'+e.mbg.mbg_ambiguous_count+' ambigu</span>':'';
    return '<div class="mbgrow"><span>'+e.date+'</span><span><b>'+e.title.slice(0,52)+'</b>'+flags+'</span><span class="num">'+e.mbg.mbg_policy_signal_count+'</span></div>';
...
function method(){
  const m=DATA.method;
  $('methodText').textContent=m.source+'. Metrik utama: '+m.primary_metric+'. Normalisasi: '+m.normalization+'. Dedup: '+m.deduplication+'. Aturan MBG: '+m.mbg_rule+'. Peringatan: '+m.warning;
  $('repoLink').innerHTML='<a href="https://github.com/himanusia/prabowo-speech-wordcount" target="_blank" rel="noreferrer">github.com/himanusia/prabowo-speech-wordcount ↗</a>';
}
function render(){method();scopeOptions();metrics();words();topics();mbg();framing();eventList();$('lexical').className='toggle'+(state.lexical?' active':'');$('lexical').setAttribute('aria-pressed',String(state.lexical));}
