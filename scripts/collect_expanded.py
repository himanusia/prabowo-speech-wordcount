...
def load_pending_ids(reasons: set[str]) -> list[str]:
    path = ROOT / "data" / "expanded" / "pending.json"
    if not path.exists():
        return []
...
def main() -> None:
    ...
    parser.add_argument(
        "--ids",
        default=None,
        help="Comma-separated video ids to fetch instead of a seed slice.",
    )
    parser.add_argument(
        "--only-pending",
        action="store_true",
        help="Fetch only ids present in data/expanded/pending.json.",
    )
    parser.add_argument(
        "--pending-reasons",
        default="IpBlocked,never_attempted",
    ...
    parser.add_argument(
        "--stop-on-block",
        action="store_true",
        help="Exit as soon as one video reports IpBlocked instead of cooling down.",
    )
    args = parser.parse_args()

    seeds = json.loads(SEED_PATH.read_text(encoding="utf-8"))["selected"]
    if args.only_pending:
        wanted = load_pending_ids(set(args.pending_reasons.split(",")))
    ...
    print(f"targets={len(selected)} sleep={args.sleep}")
    ...
    for index, seed in enumerate(selected, start=1):
        ...
        try:
            ...
            print(
                f"{entry['status'].upper()} {index}/{len(selected)} {video_id} "
            ...
            print(
                f"ERROR {index}/{len(selected)} {video_id} "
